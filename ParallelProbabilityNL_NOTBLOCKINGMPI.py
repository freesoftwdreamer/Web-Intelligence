...
def main():
    ...
    # Non-blocking gather results from all processes
    req_results = comm.Igather(results_chunk, None, root=0)
    req_zero_prob_urls = comm.Igather(zero_probability_urls_chunk, None, root=0)

    if rank == 0:
        # Wait for the non-blocking gathers to complete
        all_results = req_results.wait()
        all_zero_probability_urls = req_zero_prob_urls.wait()
