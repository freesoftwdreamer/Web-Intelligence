...
def main():
    ...
    if rank == 0:
        ...
        df_input = pd.read_csv('ATinput.csv')

        # Read the probabilities CSV file
        df_probabilities = pd.read_csv('ecommerce_probabilities_parallel_ATDATASET.csv')
    ...
    if rank == 0:
        ...
        updated_df.to_csv('NEWATinput.csv', index=False)
