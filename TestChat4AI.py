        ...
        with open(filename, mode='r', newline='') as file:
            ...
            for row in reader:
                if row:  # Ensure the row is not empty
                    urls.append(row[0])  # Assuming each row has one URL in the first column
...
async def process_url(url, server_url):
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(f"Failed to get response from llama-server for URL {url}: {response.status_code}")
            llama_response = ""
...
async def process_urls_from_csv(input_csv, output_csv, server_url):
...
if __name__ == "__main__":
    ...
    asyncio.run(process_urls_from_csv("urls.csv", "output_results.csv", server_url))
