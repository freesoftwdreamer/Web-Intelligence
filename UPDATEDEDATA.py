...
def update_ecommerce_indicator(df_input, df_probabilities):
    ...
    # Manually update the 'ecommerce' field for https://www.hertz.de to 1
    merged_df.loc[merged_df['url'] == 'https://www.hertz.de', 'ecommerce'] = 1

    # Manually update the 'ecommerce' field for https://www.notebooksbilliger.de to 1
    merged_df.loc[merged_df['url'] == 'https://www.notebooksbilliger.de', 'ecommerce'] = 1
...
def main():
    ...
    if rank == 0:
        ...
        df_input = pd.read_csv('DEinput.csv')

        # Read the probabilities CSV file
        df_probabilities = pd.read_csv('ecommerce_probabilities_parallel_DEDATASET.csv')
    ...
    if rank == 0:
        ...
        updated_df.to_csv('NEWDEinput.csv', index=False)

        logging.info("Updated input saved to 'DEinput.csv'.")
